...
CD74HC4067 lw_mux(7, 6, 5, 4);  // create a new CD74HC4067 object, UPDATE control pins with available pins
const int lw_pin = A15; // read pin for the CD74HC4067

CD74HC4067 lw_sg_mux(3, 2, 1, 0);  // create a new CD74HC4067 object, UPDATE control pins with available pins
const int lw_sg_pin = A16; // read pin for the CD74HC4067

CD74HC4067 sg_mux(8, 9, 10, 11);  // create a new CD74HC4067 object, UPDATE control pins with available pins
const int sg_pin = A17; // read pin for the CD74HC4067
...
void setup() {
    ...
    pinMode(sg_pin, INPUT); // set the initial mode of the common pin
}
...
void read_all_joints() {
  ...
  for (int i = 0; i < num_lw_sensor; i++) {
    if (lw_mux_channels[i] <= 15) {
        lw_mux.channel(lw_mux_channels[i]); // Add 2 due to MUX configuration
        analogRead(lw_pin); // Dummy read! Allows the Teensy ADC pin capacitor to charge
        sensor_data[num_potentiometer + num_press_sensor + i] = analogRead(lw_pin)/4; // read byte-sized data
    }
    else if (lw_mux_channels[i] > 15) {
        lw_sg_mux.channel(lw_mux_channels[i]-16);
  ...
  for (int i = 0; i < num_sg_sensor; i++) {
    if (sg_mux_channels[i] <= 15) {
        sg_mux.channel(sg_mux_channels[i]); // Add 2 due to MUX configuration
        analogRead(sg_pin); // Dummy read! Allows the Teensy ADC pin capacitor to charge
        sensor_data[num_potentiometer + num_press_sensor + num_lw_sensor + i] = analogRead(sg_pin)/4; // read byte-sized data
    }
    else if (sg_mux_channels[i] > 15) {
        lw_sg_mux.channel(sg_mux_channels[i]-8);
        analogRead(lw_sg_pin); // Dummy read! Allows the Teensy ADC pin capacitor to charge
...
void loop() {
      ...
      Serial.printf("%4d", sensor_data[num_potentiometer + num_press_sensor + 22]);
      Serial.println();
      Serial.print("arbitrary: ");
      Serial.printf("%4d", sensor_data[num_potentiometer + num_press_sensor + 23]);
      Serial.println();
      Serial.print("val: ");
      Serial.printf("%4d", sensor_data[num_potentiometer + num_press_sensor + 24]);
      Serial.println();
